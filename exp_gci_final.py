...
for lvl,(na,njj) in meshes.items():
    ...
    if not ff: print(f"{lvl}: FAIL",flush=True); continue
    ...
    q=0.5*rho*V**2; S=chord*0.1; Cdv=drag/(q*S); cd[lvl]=Cdv; ncell=na*njj
...
if len(cd)==3:
    ...
    if e21!=0 and (e32/e21)>0:
        ...
        json.dump({"cd":cd,"p":round(p_ord,3),"Cd_extrap":round(fext,5),"gci_pct":round(gci,3)},open("gci_final.json","w"),indent=2)
    else:
        print(f"\nGCI: monotonik degil (e32/e21={e32/e21:.2f}) - sadece Cd degerleri raporlanir")
        json.dump({"cd":cd},open("gci_final.json","w"),indent=2)
