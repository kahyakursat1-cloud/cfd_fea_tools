...
def tara() -> list[dict]:
    ...
    for f in _dosyalar():
        try:
            agac = ast.parse(f.read_text(encoding="utf-8", errors="replace"))
        except SyntaxError:
            continue
        fonk = {}
    ...
    bulgular.sort(key=lambda b: (not b["guven_yolu"],
...
def main() -> int:
    ...
    for x in b[:40]:
        ...
        isaret = "⚠ " if x["guven_yolu"] else "  "
        print(f"{isaret}{yer:36} {x['fonksiyon']:32} {x['yakalanan'][:22]:22} {x['nasil']}")
    print(f"\n**Toplam:** {len(b)} sessiz yutma; **{len(gy)}'i GÜVEN YOLUNDA** "
          "(sonucu bir sayıya/hükme dönüşen modüller)")
    if "--json" in sys.argv:
        (ROOT / "sessiz_yutma.json").write_text(
            json.dumps({"toplam": len(b), "guven_yolu": len(gy), "bulgular": b},
