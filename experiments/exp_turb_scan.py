...
def run(Ipct, ratio):
    ...
    linux_run(f"source /opt/openfoam11/etc/bashrc && unset FOAM_SIGFPE && cd {wsl} && potentialFoam -initialiseUBCs -writep >log.pot 2>&1; foamRun -solver incompressibleFluid >log.run 2>&1", 3600)
