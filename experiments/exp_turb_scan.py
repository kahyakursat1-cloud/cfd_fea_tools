...
def run(Ipct, ratio):
    ...
    subprocess.run(f'wsl bash -c "source /opt/openfoam11/etc/bashrc && unset FOAM_SIGFPE && cd {wsl} && potentialFoam -initialiseUBCs -writep >log.pot 2>&1; foamRun -solver incompressibleFluid >log.run 2>&1"',shell=True,capture_output=True,text=True,timeout=3600)
