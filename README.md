...
| 3D araç $C_d$ — V&V/UQ bandı | ⚠️ Bantlı | Model-form bandı (KARIŞIK: ölçüm + korunan öncül) — lifting %5.4 (n=1, ÜST SINIR — hiçbir çapa model hatasını sayısal hatadan ayıramıyor), attached_2d %12.0 (ÖNCÜL — tek çapa %2.8 bandı daraltmadı), bluff %9.3 (n=3, ÜST SINIR — hiçbir çapa model hatasını sayısal hatadan ayıramıyor), separated %25.0 (ÖNCÜL — tek çapa %8.8 bandı daraltmadı) |
...
| Aeroelastik — burulma diverjansı (2-yönlü FSI şeması) | ✅ Yüksek | Statik aeroelastik burulma diverjansı ↔ kapalı-form: q_div = 9819 Pa vs analitik 9817 Pa → %0.01; uç-burulma hatası %0.01 (0.5·q_div'de). Bağlaşım 0.95·q_div'de yakınsıyor, 1.05·q_div'de yakınsamıyor — eşik doğru tespit ediliyor. KISIT: 1B burulma FE ↔ quasi-steady thin-airfoil; ccx/CFD BAĞLAŞIMI DEĞİL, partisyonlu şemanın kendisi doğrulanıyor |
| Topoloji optimizasyonu — gerilme farkındalıklı | ⚠️ Kod doğrulandı, fiziksel geçerleme YOK | Gerilme farkındalıklı SIMP: 2B tepe von Mises 2.6778 → 2.482 (%7.3 azalma), 3B 0.788 → 0.762 (%3.3). Adjoint gradyanı FD ile doğrulandı (2B ~1e-7, 3B <1e-4) — bu KOD doğrulamasıdır. YOK: mesh-bağımsız topoloji, minimum özellik boyutu kısıtı, çoklu yük durumu, üretim kısıtı, ve optimize geometrinin BAĞIMSIZ yeniden-mesh + FEA ile tekrar doğrulanması. Sonuç ARAŞTIRMA seviyesidir |
| Stall / $C_{L,max}$ | ⚠️ ±2-3°, ±%15 (RANS) | beyan — kanıt dosyası yok |
| Ayrılmış akış — yeniden-yapışma uzunluğu (2D basamak) | ⚠️ Yalnız eğilim | Geriye-basamaklı akış ↔ Driver & Seegmiller 1985 (Re_H=37500): yeniden-yapışma kOmegaSST ile Xr/H=5.54 vs deney 6.26 → %-12; kEpsilon bu kurulumda sabit noktaya OTURMUYOR (20000 iterasyon, rezidüeller platoda). Duvar-çözünür aile (3 seviye, 102,400 hücre, y⁺tepe=0.048): Xr/H=5.605 → %-10.5, ayrıklaştırma bandı ±%2.39 — sapma bandtan BÜYÜK, model hatası sayısaldan ayrılabiliyor. Duvar-fonksiyonu YÖNLÜ aile (3 seviye, 84,800 hücre, y⁺ort=43): Xr/H=5.712 → %-8.7, band ±%1.7914 + u_D %1.6 → u_val %2.4 — sapma BÜYÜK, separated.wall_function ölçüme bağlandı |
| Zaman-çözünür (URANS) — girdap dökülme frekansı | ⚠️ Laminer ✅ / türbülans ❌ | Laminer silindir Re=100 (2B zaman-çözünür): St=0.16752 vs Williamson 0,164 → %+2.1; Cd=1.3249 → %-0.4. Frekans ölçümü ve URANS case yazıcısı gerçek çözücü koşusunda ÇALIŞIYOR. TÜRBÜLANSLI Re=1,4e5 (kOmegaSST, duvar-fonksiyonu, y⁺ ort=51): mekanizma koştu ama St=0.2738 deneysel plato 0,19-0,21 DIŞINDA (%+37) ve Cd %-14 — iki sapmanın yönü tutarlı: 2B kurulum span-yönü korelasyon kaybını temsil edemez. Türbülanslı zaman-çözünür sonuç ÇAPA SAYILMAZ |
